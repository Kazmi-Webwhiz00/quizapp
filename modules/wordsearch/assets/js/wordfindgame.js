...
(function (document, $, wordfind) {
  ...
  // Keep track of how many words have been found
  var foundWordCount = 0;

  var drawPuzzle = function (el, puzzle, options) {
    var output = "";
    for (var i = 0, height = puzzle.length; i < height; i++) {
      // Use a flex container for each row
      output += '<div class="puzzleRow" style="display: flex;">';
      for (var j = 0, width = puzzle[i].length; j < width; j++) {
        var letter = puzzle[i][j] || "&nbsp;";
        if (options && typeof options.createCell === "function") {
          var $cell = options.createCell(letter);
          $cell.attr("x", j).attr("y", i);
          output += $cell.prop("outerHTML");
        } else {
  ...
  var WordFindGame = function (puzzleEl, options) {
    ...
    drawPuzzle(puzzleEl, puzzle, options);
